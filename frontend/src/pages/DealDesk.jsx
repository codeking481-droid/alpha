      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-wrap items-center justify-between gap-4 animate-fadeIn">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/50 tracking-widest uppercase font-semibold">Usefulness: clients → invoices → contracts → money. No fake $103k — only your real revenue.</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pb-10 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="gold-card p-6 sm:p-8 rounded-2xl animate-slideUp">
            <div className="text-xs tracking-widest uppercase font-bold text-white/40">Total Revenue — Real</div>
            ...
            <div className="text-xs text-white/30 mt-2">{hasData ? "From your invoices" : "No invoices yet"}</div>
          </div>
          <div className="glass p-6 sm:p-8 rounded-2xl">
            <div className="text-xs tracking-widest uppercase font-bold text-white/40">Clients — Real</div>
            <div className="text-3xl sm:text-5xl font-black text-white mt-3">{clients.length}</div>
            <div className="text-xs text-white/30 mt-2">{clients.length ? "Your companies" : "Add in Command Hub"}</div>
          </div>
          <div className="glass p-6 sm:p-8 rounded-2xl">
            <div className="text-xs tracking-widest uppercase font-bold text-white/40">Invoices — Real</div>
            <div className="text-3xl sm:text-5xl font-black text-white mt-3">{invoices.length}</div>
            <div className="text-xs text-white/30 mt-2">{invoices.length ? "You created" : "0 — create below"}</div>
          </div>
          <div className="glass p-6 sm:p-8 rounded-2xl">
            <div className="text-xs tracking-widest uppercase font-bold text-white/40">Contracts — Real</div>
            <div className="text-3xl sm:text-5xl font-black text-white mt-3">{contracts.length}</div>
            <div className="text-xs text-white/30 mt-2">{contracts.length ? "Real agreements" : "0 — add below"}</div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            {selected && (
              <div className="gold-card rounded-2xl p-6 border border-[#FFD700]/20">
                ...
                <div className="text-xs text-white/60 mt-1">Status: {selected.status}</div>
                <button onClick={() => setSelected(null)} className="mt-3 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white text-xs font-bold">Close</button>
              </div>
            )}
            <InvoiceTracker />
          </div>
